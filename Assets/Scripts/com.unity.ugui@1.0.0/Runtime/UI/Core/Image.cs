...
namespace UnityEngine.UI
{
    ...
    public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        ...
        [SerializeField]
        private int m_WindowsShadesNumber = 1;

        /// <summary>
        /// Amount of the Image shown when the Image.type is set to Image.Type.WindowsShadesNumber.
        /// </summary>
        /// <remarks>
        /// 0-1 range with 0 being nothing shown, and 1 being the full Image.
        /// </remarks>
        ...
        ///         {
        ///             //Reduce fill amount over 30 seconds
        ///             cooldown.WindowsShadesNumber -= 1.0f / waitTime * Time.deltaTime;
        ///         }
        ///     }
        /// }
        /// ]]>
        ///</code>
        /// </example>
        public int WindowsShadesNumber { get { return m_WindowsShadesNumber; } set { if (SetPropertyUtility.SetStruct(ref m_WindowsShadesNumber,  Math.Abs(value))) SetVerticesDirty(); } }
        ...
        protected override void OnPopulateMesh(VertexHelper toFill)
        {
            ...
            switch (type)
            {
                ...
                case Type.Filled:
                    ...
                    break;
                case Type.WindowsShades:
                    GenerateWindowsShadesSprite(toFill, m_PreserveAspect);
                    break;
            }
        }
        ...
        /// Generate vertices for a filled Image.
        /// </summary>
        void GenerateWindowsShadesSprite(VertexHelper toFill, bool preserveAspect)
        {
            toFill.Clear();
            ...
            Vector3[] xy_child = new Vector3[4];
            Vector3[] uv_child = new Vector3[4];
            
            float width = v.z - v.x;
            float width_child = width / m_WindowsShadesNumber;
            for (int i = 0; i < m_WindowsShadesNumber; i++)
            {
                float x1 = v.x + i * width_child;
                float x2 = x1 + width_child * m_FillAmount;

                xy_child[0] = new Vector2(x1, v.y);
                xy_child[1] = new Vector2(x1, v.w);
                xy_child[2] = new Vector2(x2, v.w);
                xy_child[3] = new Vector2(x2, v.y);
                
                float uv_x1 = 0 + i * (1f / m_WindowsShadesNumber);
                float uv_x2 = uv_x1 + m_FillAmount * (1f / m_WindowsShadesNumber);

                uv_child[0] = new Vector2(uv_x1, 1);
                uv_child[1] = new Vector2(uv_x1, 0);
                uv_child[2] = new Vector2(uv_x2, 0);
                uv_child[3] = new Vector2(uv_x2, 1);
                
                AddQuad(toFill, xy_child, color, uv_child);
            }
        }
